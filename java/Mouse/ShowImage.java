...
//
import java.awt.Color;			//import java.awt.*; for classes Graphics, Color, Font, Image
...
public class ShowImage
{
	...
	public void run()
	{
		...
		label.setBounds(335,0,300,100);		//location is 335,0 and size is 300 by 100
...
// canvas - remember that we need al of the events with these listeners
class DrawingArea extends JPanel implements MouseListener, KeyListener 
{
	...
	public DrawingArea ( )   
	{
		...
		setBackground(Color.GRAY);		//satBackground(Color.RED; Red is covered. Why?
		
		/* You can use setBounds(x, y, width, height) to specify the
		 * position and size of a GUI component if you set the layout to
		 * null. Then (x,y) is the component of the upper-left corner of that
		 * component.
		 * setBounds is a shortcut for setLocation and size of the canvas
		 */
		 
		////////////////////////////////////////////////////////
		// 1. Set the location and size of the canvas
		setLocation(0,100);			//100 is needed so the repaint daoes not cover the label
		setSize(800,650);
...
public void getMyImage() 
{
	...
	catch(IOException e)
	{
		...
		e.printStackTrace();
		//since there is no System.exit(); the program does not stop 
	}
}
	...
	public void mousePressed(MouseEvent evt)
	{
		...
		ypos = evt.getY()-150;	//of the picture. Original #s were 135 and 90
		repaint();
	}
